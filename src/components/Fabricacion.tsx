...
export default function Fabricacion({ insumos, recipes, onFabricacionComplete }: FabricacionProps) {
  ...
  const handleDeleteRecipe = async (id: number) => {
    ...
    try {
      ...
      console.error(err);
      setCatalogError("Error al eliminar la fórmula/receta. Verifique si está vinculada a un producto de venta activo.");
    }
  };
  ...
  return (
    <div id="fabricacion-section" className="grid grid-cols-1 lg:grid-cols-3 gap-6 font-sans">
        ...
        <div className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm">
          ...
          <form onSubmit={handleCreateRecipe} className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-xs font-semibold text-slate-600 mb-1">Rendimiento (Unidades) *</label>
                <input
                  type="number"
                  value={recipeYield}
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-600 mb-1">Margen Ganancia (%) *</label>
                <input
                  type="number"
                  value={recipeMargin}
            ...
            <div className="border-t border-slate-100 pt-3">
              <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">Ingredientes de Despensa</label>
              ...
              <div className="flex gap-2 mb-2">
                ...
                <button
                  ...
                  className="bg-indigo-50 hover:bg-indigo-100 text-indigo-700 text-xs font-bold px-3 py-1.5 rounded-lg border border-indigo-150 cursor-pointer"
                >
                  Añadir
                </button>
              </div>
              ...
              <div className="space-y-1 max-h-[140px] overflow-y-auto mt-2">
                {tempIngredients.length === 0 ? (
                  <p className="text-[10px] text-slate-400 italic text-center py-4">Agrega insumos de arriba para componer la fórmula.</p>
                ) : (
                  tempIngredients.map((ing, idx) => (
                    <div key={idx} className="flex items-center justify-between p-2 bg-slate-50 rounded-lg border border-slate-100 text-xs">
                      <span className="font-medium text-slate-800">{ing.insumoName}</span>
            ...
            {tempIngredients.length > 0 && (
              <div className="bg-indigo-50/40 border border-indigo-100 p-4 rounded-xl space-y-1.5 text-xs">
                <div className="flex justify-between">
                  <span className="text-slate-500">Costo total de tanda:</span>
                  <span className="font-mono font-bold text-slate-800">${tempTotalCost.toFixed(2)}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-slate-500">Costo Unitario Pieza:</span>
                  <span className="font-mono font-bold text-slate-850">${tempCostPerPiece.toFixed(2)}</span>
                </div>
                <div className="flex justify-between border-t border-indigo-100/50 pt-1.5">
                  <span className="font-bold text-slate-700">Precio Sugerido Venta:</span>
                  <span className="font-mono font-bold text-indigo-700 bg-indigo-50 px-2 rounded">${tempSuggestedPrice.toFixed(2)}</span>
                </div>
                <p className="text-[9px] text-indigo-500 mt-1">Margen aplicado: {tempMargin}% sobre costo de fabricación.</p>
              </div>
            )}
            ...
            <button
              ...
              className={`w-full font-semibold py-2.5 rounded-xl text-sm transition-colors text-white cursor-pointer ${
                tempIngredients.length === 0 
      ...
      <div className="lg:col-span-2 space-y-6">
        ...
        {/* Active formulas */}
        <div className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm">
          <h2 className="text-sm font-bold text-slate-900 uppercase tracking-wider mb-4">Fórmulas del Taller</h2>

          {catalogError && (
            <div className="bg-rose-50 border border-rose-200 text-rose-700 px-4 py-2.5 rounded-xl text-xs font-semibold flex items-center justify-between mb-4 animate-in fade-in duration-200">
          ...
          {recipes.length === 0 ? (
            <p className="text-xs text-slate-400 italic py-8 text-center">No hay recetas cargadas. Crea una fórmula a la izquierda.</p>
          ) : (
              ...
              {recipes.map(recipe => (
                ...
                >
                  <div>
                    <div className="flex items-start justify-between">
                      ...
                      {deletingId === recipe.id ? (
                        <div className="flex items-center gap-1.5 bg-rose-50 border border-rose-200 p-1 rounded-lg animate-in fade-in zoom-in duration-200" onClick={(e) => e.stopPropagation()}>
                          <span className="text-[9px] font-bold text-rose-700 px-0.5">¿Eliminar?</span>
                          <button
                            onClick={(e) => {
                              e.stopPropagation();
                            ...
                            className="px-1.5 py-0.5 text-[9px] font-bold bg-rose-600 hover:bg-rose-700 text-white rounded cursor-pointer"
                          >
                            Sí
                          </button>
                          <button
                            ...
                            className="px-1.5 py-0.5 text-[9px] font-bold bg-slate-200 hover:bg-slate-300 text-slate-700 rounded cursor-pointer"
                          >
                            No
                          </button>
                        </div>
                    ...
                    <p className="text-[10px] text-indigo-600 font-bold bg-indigo-50/80 px-2 py-0.5 rounded-full inline-block mt-1">
                      Rinde: {recipe.yield} u. / Margen: +{recipe.marginPercent}%
                    </p>

                    <div className="mt-3 space-y-1 text-xs">
                      <p className="text-slate-500 flex justify-between font-mono">
                        <span>Costo Pieza:</span>
                        <b>${recipe.costPerPiece.toFixed(2)}</b>
                      </p>
                      <p className="text-slate-500 flex justify-between font-mono">
                        <span>Precio Venta:</span>
                        <b className="text-slate-800">${recipe.suggestedPrice.toFixed(2)}</b>
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="border-t border-slate-100 mt-3 pt-3 flex items-center justify-between text-[10px] text-slate-400">
                    ...
                    <span className="font-semibold text-indigo-600 hover:underline">Seleccionar para Fabricar &rarr;</span>
                  </div>
                </div>
              ))}
            </div>
        ...
        {activeRecipe && (
          <div className="bg-gradient-to-r from-slate-900 to-indigo-950 text-white p-6 rounded-2xl border border-slate-800 shadow-md space-y-4">
            <div className="flex items-center justify-between border-b border-slate-800 pb-3">
              <div className="flex items-center gap-2">
                <Sliders className="w-5 h-5 text-amber-400" />
                <div>
                  <h3 className="text-sm font-bold">Taller de Fabricación Activo</h3>
                  <p className="text-[10px] text-slate-400">Produciendo: <b>{activeRecipe.name}</b></p>
                </div>
              </div>
              <button
            ...
            <div className="space-y-2">
              <div className="flex justify-between text-xs">
                <span className="text-slate-300">Tandas a fabricar:</span>
                <span className="font-bold text-amber-400 text-sm font-mono">{batchesToProduce} tanda(s) (= {activeRecipe.yield * batchesToProduce} unidades terminadas)</span>
              </div>
              <input
                type="range"
                min="1"
            ...
            <div className="bg-slate-900/60 p-4 rounded-xl border border-slate-800 space-y-2">
              <h4 className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Chequeo de Disponibilidad de Despensa</h4>
              
              <div className="space-y-1.5">
                {abastecimiento?.analysis.map((item, idx) => (
                  <div key={idx} className="flex items-center justify-between text-xs">
                    <span className="text-slate-300">{item.insumoName}</span>
                    <div className="flex items-center gap-2 font-mono">
                      <span className="text-[10px] text-slate-400">Req: {item.required}{item.unit} / Tienes: {item.has}{item.unit}</span>
                      {item.ok ? (
                        <span className="text-emerald-400 font-bold flex items-center gap-0.5">
                          <CheckCircle2 className="w-3.5 h-3.5 inline" /> Ok
                        </span>
                      ) : (
                        <span className="text-rose-500 font-bold flex items-center gap-0.5">
                          <AlertTriangle className="w-3.5 h-3.5 inline animate-pulse" /> Faltante
                        </span>
                      )}
                    </div>
                  </div>
            ...
            <div className="flex items-center justify-between pt-2">
              ...
              <button
                ...
                className={`flex items-center gap-1.5 font-bold py-2.5 px-5 rounded-xl text-xs transition-colors shadow cursor-pointer ${
