...
export default function MapaClientes({ clients, onClientAdded, onClientDeleted }: MapaClientesProps) {
  ...
  useEffect(() => {
    ...
    map.on("click", (e: any) => {
      ...
      const tempIcon = L.divIcon({
        html: `<div className="flex items-center justify-center w-8 h-8 rounded-full bg-emerald-500 text-white shadow-xl animate-bounce border-2 border-white">
      ...
      if (client.lat && client.lng) {
        const clientIcon = L.divIcon({
          html: `<div className="flex flex-col items-center">
                   <div className="flex items-center justify-center w-8 h-8 rounded-full bg-indigo-600 text-white shadow-lg border-2 border-white hover:scale-115 transition-transform">
                     <span className="text-xs">👤</span>
                   </div>
                   <div className="bg-indigo-950 text-white text-[10px] px-1.5 py-0.5 rounded shadow mt-1 whitespace-nowrap font-medium border border-indigo-800">
                     ${client.name}
                   </div>
                 </div>`,
        ...
        const marker = L.marker([client.lat, client.lng], { icon: clientIcon })
          ...
          .bindPopup(`
            <div className="p-1 font-sans">
              <h3 className="font-bold text-slate-900 border-b pb-1 mb-1 text-sm">${client.name}</h3>
              ${client.phone ? `<p className="text-xs text-slate-600 flex items-center gap-1">📞 ${client.phone}</p>` : ""}
              ${client.email ? `<p className="text-xs text-slate-600 flex items-center gap-1">✉️ ${client.email}</p>` : ""}
              <p className="text-[10px] text-slate-400 mt-1">Coordenadas: ${client.lat.toFixed(4)}, ${client.lng.toFixed(4)}</p>
            </div>
          `);
        ...
        if (mapRef.current) {
          ...
          if (L) {
            ...
            const myIcon = L.divIcon({
              html: `<div className="flex items-center justify-center w-8 h-8 rounded-full bg-blue-500 text-white shadow-xl animate-bounce border-2 border-white">
                       <span className="text-xs">🌟</span>
                     </div>`,
  ...
  return (
    <div id="mapa-section" className="grid grid-cols-1 lg:grid-cols-3 gap-6 font-sans">
      ...
      <div className="lg:col-span-1 flex flex-col gap-6">
        ...
        <div className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-xs font-semibold text-slate-600 mb-1">Latitud</label>
                <input
                  type="number"
                  step="any"
                  value={lat}
                  ...
                  className="w-full px-3 py-2 text-xs border border-slate-200 rounded-lg bg-slate-50 focus:outline-none"
                />
              </div>
              <div>
                <label className="block text-xs font-semibold text-slate-600 mb-1">Longitud</label>
                <input
                  type="number"
                  step="any"
                  value={lng}
            ...
            <button
              ...
              disabled={loading}
              className="w-full flex items-center justify-center gap-2 bg-indigo-50 hover:bg-indigo-100/80 active:bg-indigo-100 text-indigo-700 font-semibold py-2 px-4 rounded-lg transition-colors text-xs cursor-pointer"
            >
        ...
        <div className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm flex-1 flex flex-col min-h-[300px]">
          <h2 className="text-sm font-bold text-slate-900 mb-3 uppercase tracking-wider">Directorio de Clientes</h2>
          <div className="space-y-2 overflow-y-auto max-h-[350px] flex-1 pr-1">
            {clients.length === 0 ? (
              <p className="text-xs text-slate-400 text-center py-8">No hay clientes registrados aún.</p>
            ) : (
              clients.map(client => (
                <div
                  key={client.id}
                  onClick={() => handleFlyTo(client)}
                  className="p-3 bg-slate-50 hover:bg-slate-100 border border-slate-100 rounded-xl transition-all cursor-pointer flex items-center justify-between group"
                >
                  <div className="flex-1 min-w-0 pr-2">
                    <h3 className="text-xs font-bold text-slate-800 truncate">{client.name}</h3>
                    <div className="flex flex-col gap-0.5 mt-1 text-[10px] text-slate-500">
                      ...
                      {client.lat && <span className="flex items-center gap-1"><MapPin className="w-2.5 h-2.5 text-indigo-500" /> Ver en Mapa (centrar)</span>}
                    </div>
                  </div>
                  <button
      ...
      <div className="lg:col-span-2 flex flex-col gap-4">
        <div className="bg-white p-2.5 rounded-2xl border border-slate-100 shadow-sm h-[600px] flex flex-col">
          <div className="p-3 bg-slate-50 rounded-xl border border-slate-100/80 mb-2 flex flex-col md:flex-row md:items-center justify-between gap-2">
            <div>
              <h3 className="text-xs font-bold text-slate-800">Mapa Georreferencial de Despachos</h3>
              <p className="text-[10px] text-slate-500 mt-0.5">Haz clic en cualquier parte del mapa para fijar una coordenada exacta de cliente.</p>
            </div>
            <span className="text-[10px] bg-indigo-50 text-indigo-700 font-bold px-2 py-1 rounded-full self-start md:self-auto border border-indigo-100">
          ...
          <div
            ref={mapContainerRef}
            className="w-full flex-1 rounded-xl shadow-inner border border-slate-200 overflow-hidden relative"
            style={{ zIndex: 1 }}
          >
