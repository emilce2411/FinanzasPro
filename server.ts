...
async function seedUserDemoData(userId: number) {
  try {
    // 0. Check if user is already marked as seeded to prevent overriding explicit wipes/clears
    const userRecord = await db.select().from(users).where(eq(users.id, userId)).then(res => res[0]);
    if (!userRecord || userRecord.isSeeded === 1) {
      return;
    }
    ...
    if (existingInsumos.length > 0) {
      await db.update(users).set({ isSeeded: 1 }).where(eq(users.id, userId));
      return;
    }
...
const AUTH_SECRET = "finanzas_pro_super_secret_token_key_2026";

function hashPassword(password: string): string {
  const salt = "finanzas_pro_salt_secure_2026";
...
async function startServer() {
  ...
  app.post("/api/auth/register", async (req, res) => {
    try {
      const { email, password, displayName, bizName } = req.body;
      if (!email || !password || !displayName || !bizName) {
      ...
      const uid = `custom:${emailNormalized}`;
      ...
      // Auto-seed demo data for isolation exploration
      await seedUserDemoData(newUser.id);
      ...
      res.status(201).json({
        ...
        user: {
          ...
          bizName: newUser.bizName,
        }
      });
    } catch (err: any) {
      console.error("Error registering user custom auth:", err);
      res.status(500).json({ error: "No se pudo crear la cuenta de usuario." });
    }
  });
  ...
  app.post("/api/auth/login", async (req, res) => {
    try {
      ...
      // Find user
      const foundUsers = await db.select().from(users).where(eq(users.email, emailNormalized));
      ...
      if (!dbUser.password) {
        return res.status(400).json({ error: "Esta cuenta se registró usando otro método. Intenta iniciar sesión con Google." });
      }

      // Verify password
      const hashedIn = hashPassword(password);
      if (dbUser.password !== hashedIn) {
        return res.status(401).json({ error: "Correo o contraseña incorrectos." });
      }
      ...
      res.json({
        ...
        user: {
          uid: dbUser.uid,
          email: dbUser.email,
          displayName: dbUser.bizName || "Usuario FinanzasPro",
          bizName: dbUser.bizName,
        }
      });
    } catch (err: any) {
      console.error("Error signing in user custom auth:", err);
      res.status(500).json({ error: "No se pudo iniciar sesión. Intente nuevamente." });
    }
  });
  ...
  // Reset or clear database from the cloud for this user (Clear ledger and optionally re-seed defaults)
  app.post("/api/transactions/reset", requireAuth, async (req: AuthRequest, res) => {
    try {
      const userId = req.user!.id;
      const { mode } = req.body || {}; // 'clear' (empty start) or 'reset' (with demo data)
      ...
      if (mode === "clear") {
        // Mark as seeded/initialized so it stays completely empty and doesn't auto-re-seed on subsequent syncs
        await db.update(users).set({ isSeeded: 1 }).where(eq(users.id, userId));
        res.json({ success: true, message: "Base de datos vaciada por completo. Listo para comenzar desde cero." });
      } else {
        // Force re-seed by resetting isSeeded to 0 first
        await db.update(users).set({ isSeeded: 0 }).where(eq(users.id, userId));
        // Re-seed demo data
      ...
      res.status(500).json({ error: "No se pudo procesar la base de datos de la nube" });
    }
  });
